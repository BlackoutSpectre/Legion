...
public class Map<T extends AbstractNode> implements Serializable{
    ...
    /**the spacing the path should be away from unwalkable areas. */
     ...
     * @param nodeFactory 
     */
    public Map(int width, int higth, NodeFactory nodeFactory, int spacing) {
    ...
    /**
     * get the size of the tile map
     * @return int[0] = x axis, int[1] = y axis
     */
    public int[] getMapSize()
    {
        int[] size =  {nodes[0].length,nodes.length};
        return size;
    }
            ...
            for (int i = 0; i < adjacentNodes.size(); i++) {
                ...
                if (!openList.contains(currentAdj)) { // node is not in openList
                    ...
                    if (currentAdj.getgCosts() > currentAdj.calculategCosts(current) && // costs from current node are cheaper than previous costs
                            hasSpace(currentAdj.getxPosition(),currentAdj.getyPosition())) { // makes sure that the node has the appropriate space
                        currentAdj.setPrevious(current); // set current node as previous for this node
                        currentAdj.setgCosts(current); // set g costs of this node (costs from start to this node)
                    }
                }
    ...
    private List<T> getAdjacent(T node) {
        ...
        if (x > 0) {
            ...
            if (temp.isWalkable() && !closedList.contains(temp)&&hasSpace(temp)) {
    ...
    {
        return ((yPos>=0&&yPos<nodes.length)&&(xPos>=0&&xPos<nodes[0].length));
    }
    ...
    public boolean hasSpace(int xPos, int yPos)
    {
        for (int y = yPos-spacing; y<=yPos+spacing; y++)
            ...
            {
                if (isInBounds(x,y)&&nodes[x][y].isWalkable())
                {
    ...
    public boolean hasSpace(T temp)
    {
        return hasSpace(temp.getxPosition(),temp.getyPosition());
    }
